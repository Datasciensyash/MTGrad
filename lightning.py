...
from model.diffusion import GaussianDiffusion, MTDiffusion, MTConditioningData
...
class TrainingEngineDiff(pl.LightningModule):
    def __init__(
        self,
        config: dict,
        model: MTDiffusion,
    ...
    def training_step(self, batch: MTDataSample, batch_idx):
        loss = self.model(
            resistivity=batch.resistivity,
            conditioning=MTConditioningData(
                **{k: v for k, v in batch.__dict__.items() if k != "resistivity"}
            )
        )
    ...
    def validation_step(self, batch: MTDataSample, batch_idx):
        resistivity_predicted = self.model.denoise(
            conditioning=MTConditioningData(
                **{k: v for k, v in batch.__dict__.items() if k != "resistivity"}
            )
        )

        return resistivity_predicted, batch.resistivity
